...
let simpler_sum xs = List.fold xs ( + ) 0 
let simpler_product xs = List.fold xs ( * ) 1 
...
let range from to_ = List.init (to_ - from)  
...
let print_int_list xs = List.iter xs print_endline;;
